document.addEventListener('DOMContentLoaded', () => {
  ...
  if (modal && modalTitle && modalItems && closeBtn) {
...
function renderOrder() {
  ...
  orderItems.forEach((item, index) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button onclick="voidItem(${index})">Void</button>
      </td>
    `;
...
function showCategory(categoryName) {
  ...
  filtered.forEach(item => {
    ...
    li.onclick = () => addToOrder(item.name, item.price, item.product_id);
...
// ✅ Fixed: Wire up category buttons
