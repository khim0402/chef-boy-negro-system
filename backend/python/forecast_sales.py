...
TABLE_SALES = "sales"
TABLE_PRODUCT_FORECAST = "product_forecast"     # columns: product_id, date, qty
TABLE_SALES_FORECAST = "sales_forecast"         # columns: date, forecast_amount
TABLE_FORECAST_METRICS = "forecast_metrics"     # columns: run_at, model_name, rmse, mae, mape, processed_products
...
def forecast_next_days(df_series: pd.DataFrame, model, horizon=FORECAST_HORIZON_DAYS):
    ...
    for i in range(1, horizon + 1):
        ...
        if model is not None:
            ...
            yhat = float(np.mean(last_values))
...
def write_sales_forecast(forecast_df: pd.DataFrame):
    ...
    with engine.begin() as conn:
        conn.execute(text("DELETE FROM sales_forecast WHERE date >= :start_date"),
                     {"start_date": pd.to_datetime(forecast_df["date"]).min().date()})
        rows = [
            {
                "date": pd.to_datetime(r["date"]).date(),
                "forecast_amount": float(r["forecast_amount"]),
                "model_used": MODEL_NAME
            }
            for _, r in forecast_df.iterrows()
        ]
        conn.execute(
            text("INSERT INTO sales_forecast (date, forecast_amount, model_used) VALUES (:date, :forecast_amount, :model_used)"),
            rows
        )
...
def write_product_forecast(product_id: int, forecasts: list):
    if not forecasts:
        return
    with engine.begin() as conn:
        conn.execute(text("DELETE FROM product_forecast WHERE product_id = :pid AND forecast_date >= :start_date"),
                     {"pid": product_id, "start_date": min(f[0] for f in forecasts)})
        rows = [
            {
                "product_id": product_id,
                "forecast_date": f[0],
                "forecast_qty": int(round(f[1])),
                "model_used": MODEL_NAME
            }
            for f in forecasts
        ]
        conn.execute(
            text("INSERT INTO product_forecast (product_id, forecast_date, forecast_qty, model_used) VALUES (:product_id, :forecast_date, :forecast_qty, :model_used)"),
            rows
        )
...
def write_forecast_metrics(rmse: float, mae: float, mape: float, processed_products: int):
    with engine.begin() as conn:
        conn.execute(
            text("""INSERT INTO forecast_metrics (model_used, mape, rmse, mae, trained_on, forecast_horizon)
                    VALUES (:model_used, :mape, :rmse, :mae, CURRENT_DATE, :horizon)"""),
            {
                "model_used": MODEL_NAME,
...
def run_pipeline(inventory_slice: pd.DataFrame | None = None) -> dict:
    ...
    for _, row in inventory_df.iterrows():
        ...
        if model is None:
            forecasts = [(date.today() + timedelta(days=i), float(y.mean())) for i in range(1, FORECAST_HORIZON_DAYS + 1)]
        else:
